...
const Diff = (() => {
  ...
  function onScanFinished(data) {
    ...
    if (!data.ok || !_compareItem) {
      ...
      _setScanMessage(err ? err.message : '재학생 파일을 확인할 수 없습니다.');
      _el('btn-scan-diff').disabled = false;
      return;
    }
    ...
    _setScanMessage('재학생 파일의 시작행을 확인해 주세요.');
  ...
  function onFinished(data) {
    ...
    if (!data.ok) {
      _setRunBadge('err', '오류');
      const err = (data.logs || []).find(l => l.level === 'error');
      _setRunInfo(err ? err.message : '실행 실패');
      return;
    }
  ...
  function onFailed(error) {
    ...
    _el('btn-run-diff').disabled = false;
    _setRunBadge('err', '오류');
    _setRunInfo(`예기치 못한 오류: ${error}`);
    toast('명단 비교 오류: ' + error, 'err');
  }

  function onPreviewLoaded(payload) {
    _previewData = payload;
    const info = `파일: ${payload.source_file || '-'} | 시트: ${payload.sheet_name || '-'} | 헤더행: ${payload.header_row || '-'} | 시작행: ${payload.data_start_row || '-'}`;
    _el('diff-preview-file-info').textContent = info;
    _el('diff-preview-warn').textContent = payload.truncated ? `${payload.rows.length}행까지만 표시합니다.` : '';
