...
const Diff = (() => {
  ...
  function onFinished(data) {
    ...
    _setRunBadge(unresolvedCount > 0 ? 'warn' : 'ok', unresolvedCount > 0 ? '경고' : '완료');
    _setRunInfo('명단 비교 결과를 아래에서 확인해 주세요.');
    const runWarns = (data.logs || []).filter(l => l.level === 'warn').map(l => l.message);
    if (runWarns.length) _showWarnCard('diff-run-warn-card', runWarns, 'warn');
  ...
  function onPreviewLoaded(payload) {
    ...
    const info = `파일: ${payload.source_file || '-'} | 시트: ${payload.sheet_name || '-'} | 헤더행: ${payload.header_row || '-'} | 시작행: ${payload.data_start_row || '-'} | 실제 ${actualCount}행`;
    _el('diff-preview-file-info').textContent = info;
    _el('diff-preview-warn').textContent = payload.truncated ? `${displayedCount}행만 표시합니다.` : '';
    const table = _el('diff-preview-table');
    if (!table) return;
    const cols = payload.columns || [];
    const rows = payload.rows || [];
    const startRow = payload.start_row || 1;
    const issueSet = new Set(payload.issue_rows || []);
    const dataStart = payload.data_start_row || 1;
    table.querySelector('thead').innerHTML = '<tr><th style="width:40px;color:#94A3B8;font-weight:600;text-align:center">#</th>' + cols.map(c => `<th>${_escHtml(c)}</th>`).join('') + '</tr>';
    if (!rows.length) {
      ...
      table.querySelector('tbody').innerHTML = rows.map((r, i) => {
        ...
        const cls = excelRow < dataStart ? 'row-prestart' : (issueSet.has(excelRow) ? 'row-warn' : '');
        return `<tr class="${cls}"><td style="color:#94A3B8;font-size:11px;text-align:center;user-select:none">${excelRow}</td>${r.map(v => `<td>${_escHtml(v)}</td>`).join('')}</tr>`;
      }).join('');
    }
  ...
  async function _requestPreview() {
    ...
    const params = {
      ...
      header_row: _compareItem.header_row || 1,
  ...
  function _showWarnCard(id, messages, mode = 'warn') {
    const el = _el(id);
    if (!el) return;
    const lines = Array.from(new Set((Array.isArray(messages) ? messages : []).map(v => String(v || '').trim()).filter(Boolean)));
    if (!lines.length) {
      _hideWarnCard(id);
      return;
    }
    ...
    const body = mode === 'error'
      ? lines.slice(0, 3).map(msg => `<div class="warn-line">• ${_escHtml(msg)}</div>`).join('') + (lines.length > 3 ? `<div class="warn-line">외 ${lines.length - 3}건</div>` : '')
      : '';
